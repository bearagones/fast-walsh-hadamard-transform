...
vector<int> FWHT(const string& type, vector<int> matrix, bool bitRev) {
...
void timeFWHT(bool bitRev) {
    ...
    if (numRows >= numColumns) {
        if (bitRev) {
            FWHT("column", bitReversal(x), bitRev);
        } else {
            FWHT("column", x, bitRev);
        }
    } else {
        if (bitRev) {
            FWHT("row", bitReversal(transpose(x)), bitRev);
        } else {
            FWHT("row", transpose(x), bitRev);
        }
    }
